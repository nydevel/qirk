...
import org.wrkr.clb.chat.model.cassandra.TaskMessage;
import org.wrkr.clb.chat.model.cassandra.TaskMessage_;

import com.datastax.oss.driver.api.core.cql.PreparedStatement;


@Component
public class CassandraTaskMessageRepo extends BaseAttachedChatMessageRepo {

    private static final String CQL_SELECT_STATEMENT = String.format(
            "SELECT %s, %s, %s FROM %s WHERE %s = ? AND %s < ? ORDER BY %s DESC LIMIT ?",
            TaskMessage_.senderId, TaskMessage_.timestamp, TaskMessage_.message, // select columns
            TaskMessage_.TABLE_NAME, TaskMessage_.chatId, TaskMessage_.timestamp, // filter columns
            TaskMessage_.timestamp); // order by column

    private static final String CQL_INSERT_STATEMENT = String.format(
            "INSERT INTO %s (%s, %s, %s, %s, %s) VALUES (?, ?, ?, ?, ?) IF NOT EXISTS", TaskMessage_.TABLE_NAME,
            TaskMessage_.uuid, TaskMessage_.chatId, TaskMessage_.senderId, TaskMessage_.timestamp,
