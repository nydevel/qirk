...
import org.wrkr.clb.chat.model.cassandra.ProjectMessage;
import org.wrkr.clb.chat.model.cassandra.ProjectMessage_;

import com.datastax.oss.driver.api.core.cql.PreparedStatement;


@Component
public class CassandraProjectMessageRepo extends BaseAttachedChatMessageRepo {

    private static final String CQL_SELECT_STATEMENT = String.format(
            "SELECT %s, %s, %s FROM %s WHERE %s = ? AND %s < ? ORDER BY %s DESC LIMIT ?",
            ProjectMessage_.senderId, ProjectMessage_.timestamp, ProjectMessage_.message, // select columns
            ProjectMessage_.TABLE_NAME,
            ProjectMessage_.chatId, ProjectMessage_.timestamp, // filter columns
            ProjectMessage_.timestamp); // order by column

    private static final String CQL_INSERT_STATEMENT = String.format(
            "INSERT INTO %s (%s, %s, %s, %s, %s) VALUES (?, ?, ?, ?, ?) IF NOT EXISTS",
            ProjectMessage_.TABLE_NAME,
            ProjectMessage_.uuid, ProjectMessage_.chatId, ProjectMessage_.senderId, ProjectMessage_.timestamp,
            ProjectMessage_.message);
