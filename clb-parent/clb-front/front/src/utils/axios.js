...
instance.interceptors.response.use(null, error => {
  ...
  if (errorIsInvalidCsrfToken(error)) {
    ...
    errorIsLicenseNotAccepted(error) &&
    ![paths.Login.toPath(), paths.UpdatedLicense.toPath()].some(
      url => url === pathname
    )
  ) {
    window.location.href = `${paths.UpdatedLicense.toPath()}?${queryString.stringify(
      {
        bounce_to: pathname
      }
    )}`;

    throw new Error("Error: You did not agree with updated license");
