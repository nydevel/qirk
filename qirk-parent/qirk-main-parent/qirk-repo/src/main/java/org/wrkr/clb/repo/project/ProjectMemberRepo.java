...
public class ProjectMemberRepo extends JPABaseDeletingRepo<ProjectMember> {
    ...
    public ProjectMember getNotFiredByOrganizationMemberAndProject(OrganizationMember organizationMember, Project project) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<ProjectMember> query = cb.createQuery(ProjectMember.class);

        Root<ProjectMember> memberRoot = query.from(ProjectMember.class);

        query.where(cb.equal(memberRoot.get(ProjectMember_.organizationMember), organizationMember),
                cb.equal(memberRoot.get(ProjectMember_.project), project),
                cb.equal(memberRoot.get(ProjectMember_.fired), false));
        return getSingleResultOrNull(query);
    }

    public ProjectMember getNotFiredByOrganizationMemberAndProjectId(OrganizationMember organizationMember, Long projectId) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<ProjectMember> query = cb.createQuery(ProjectMember.class);
    ...
    public ProjectMember getNotFiredByOrganizationMemberAndProjectUiId(OrganizationMember organizationMember,
            String projectUiId) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<ProjectMember> query = cb.createQuery(ProjectMember.class);
    ...
    public List<ProjectMember> listNotFiredByOrganizationMember(OrganizationMember organizationMember) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        CriteriaQuery<ProjectMember> query = cb.createQuery(ProjectMember.class);

        Root<ProjectMember> projectMemberRoot = query.from(ProjectMember.class);

        query.where(cb.equal(projectMemberRoot.get(ProjectMember_.organizationMember), organizationMember));
        return getResultList(query);
    }
}
