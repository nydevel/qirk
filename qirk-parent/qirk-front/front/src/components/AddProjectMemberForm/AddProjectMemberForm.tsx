...
type OwnProps = {
  onInvitePosted?: Function;
};

type AddProjectMemberFormProps = OwnProps &
  RouteComponentProps<{ project_uiid: string }> &
  ConnectedProps<typeof connector>;

const AddProjectMemberForm: FC<AddProjectMemberFormProps> = ({
  addProjectMembersDispatch,
  projectId,
  onInvitePosted,
  user,
  ...
  const [searchReqTimeout, setSearchReqTimeout] = useState<any>(null);
  const [selectedMember, setSelectedMember] = useState<any>(null);
  const [options, setOptions] = useState<any>([]);
  const [searchStr, setSearchStr] = useState("");

  const handleClickOutside = (e: any /* todo */) => {
    if (
      wrapperRef &&
      wrapperRef.current &&
  ...
  const processForm = async (selectedMember: any) => {
    if (!selectedMember) {
      return false;
    }
    ...
    if (selectedMember.user && selectedMember.user.is_member) {
      try {
        ...
        if (responseIsStatusOk(response)) {
          ...
          setOptions((old: any) =>
            old.filter(
              (opt: any /* todo */) => opt.value !== selectedMember.value
            )
          );
      ...
      try {
        ...
        setSelectedMember(null);
        setOptions(
          options.filter((opt: any) => opt.value !== selectedMember.value)
        );
      }
      ...
      try {
        ...
        if (responseIsStatusOk(response)) {
          if (responseIsEmailSent(response)) {
            ...
            if (inviteData && inviteData.id && onInvitePosted) {
              onInvitePosted({
                ...inviteData,
                email: searchStr,
                sender: { ...user }
        ...
        <div style={{ display: "flex" }}>
          <div style={{ flex: 1 }}>
            <TextInput
              autoComplete="off"
              value={searchStr}
              onChange={(e: any /**todo */) => setSearchStr(e.target.value)}
              label={t("Add member")}
          ...
          >
            <fieldset>
              <Checkbox
                ...
                checked={allowWrite}
                onChange={(e: any) => setAllowWrite(e.target.checked)}
              ...
              <Checkbox
                disabled={requestInProgress}
                checked={isManager}
                onChange={(e: any) => setIsManager(e.target.checked)}
...
export default withRouter(connector(AddProjectMemberForm));
