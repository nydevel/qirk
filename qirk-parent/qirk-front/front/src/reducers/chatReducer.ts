...
import { ChatSender } from "../utils/types/ChatSender";
import { ChatMessage } from "../utils/types/ChatMessage";

export type ChatReducerState = {
  chatTokenRequestStatus: RequestStatus;
  chatToken: string;
  chatIV: string;
  chatId: null | number;
  ...
  senders: ChatSender[];
  earliestMessageTimeOnLoadBefore: number;
  socket: null | WebSocket;
  resendingUncomfirmedMessagesRequired: boolean;
  tokenRefreshRequired: boolean;
  lastTimeTokenFetched: number;
  socketTokenNotificationRequired: boolean;
  socketTokenNotificationType: null | any;
  loadBeforeRequired: boolean;
  ignoreMatchingEarliestMessageTimeOnloadBefore: boolean;
  isLoading: boolean;
};

const INITIAL_STATE: ChatReducerState = {
  chatTokenRequestStatus: RequestStatus.NOT_REQUESTED,
  chatToken: "",
  chatIV: "",
  chatId: null,
  chatType: null,
    ...
    case constants.CHAT_CONFIRM_MESSAGES_DELIVERY_BY_EXTERNAL_UUIDS:
      return {
        ...
        messages: state.messages.map(m =>
          action.payload.some((uuid: any) => uuid === m.external_uuid)
    ...
    case constants.DELETE_CHAT_MESSAGES_BY_EXTERNAL_UUID_LIST:
      return {
        ...
        messages: state.messages.filter(
          cMessage =>
            !action.payload.some(
              (uuidToSearch: string) => uuidToSearch === cMessage.external_uuid
            )
        )
      };
